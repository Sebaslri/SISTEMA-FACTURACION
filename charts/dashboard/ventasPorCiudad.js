...
document.addEventListener("DOMContentLoaded", function () {
    var map = L.map('ventasMap').setView([-2.17, -79.9], 8);
    ...
    fetch("../charts/dashboard/ventasPorCiudad.php")
        .then(response => response.json())
        .then(data => {
            if (!data || data.length === 0) {
                console.warn("No se recibieron datos de ventas");
                return;
            }

            let maxVentas = Math.max(...data.map(item => item.ventas));

            data.forEach(item => {
                // Buscar coordenadas en el diccionario cantones
                let canton = cantones.find(c => c.name.toLowerCase() === item.ciudad.toLowerCase());

                if (canton) {
                    let [lat, lng] = canton.latlng;
                    let radius = 5 + (item.ventas / maxVentas) * 35;

                    // Colores: rojo si está por encima de la mitad del máximo, azul si no
                    let color = item.ventas > maxVentas / 2 ? "red" : "blue";
                    let fillColor = item.ventas > maxVentas / 2 ? "salmon" : "lightblue";
                    ...
                    .addTo(map)
                    .bindPopup(`<b>${item.ciudad}</b><br>Ventas: $${item.ventas.toFixed(2)}`);
                } else {
