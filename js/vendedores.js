...
function activaDesactivarVendedor(id) {
    if (confirm("¿Estás seguro de que deseas cambiar el estado del Vendedor?")) {
        $.ajax({
            url: '',
            method: 'POST',
            data: { toggle_vendedor: id },
            success: function(response) {
                const trimmed = $.trim(response);
                const parts = trimmed.split("-");
                
                if (parts[0] === "1") {
                    const estado = parts[1] === "ON" ? "VendorOn" : "VendorOff";
                    window.location.href = "../views/vendedores.php?status=" + estado;
                } else {
